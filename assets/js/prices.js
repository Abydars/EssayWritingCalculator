...
jQuery.extend({
    ew: {
        onFormInitialized: function () {
            console.log('ew: onFormInitialized');
        }
    }
});
...
function moveToNextStep(f) {
    var $ = jQuery;
    var onsubmit = $("#steps .step[data-step=" + currentstep + "]").data("onsubmit");
    var force = ((typeof f !== "undefined") && (f === true));

    if (totalsteps > currentstep) {
        if ((typeof onsubmit !== "undefined") && !force) {
            eval(onsubmit);
        } else {
            currentstep++;
...
function resendConfirmation(url) {
    ...
    animateFormTop();
    disableFormWithMessage("Sending confirmation email...");

    var data = {
        action: 'ew_resend_confirmation',
        url: url
    };
...
function showErrorMessage(msg) {
    ...
    $("#steps > .alert-danger").html(msg).show();
}
