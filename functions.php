...
function check_coupon() {
	if(isset($_REQUEST['coupon'])) {
		$coupons = get_posts(array('post_type' => 'ew_coupon', 'nopaging' => 1, 'meta_key' => 'code', 'meta_value' => $_REQUEST['coupon']));
		$amount = 0;
				...
				<table class="form-table">
					<tbody>
						<tr valign="top">
							...
							<td>
								<input type="text" name="ew[paypal_email]" value="<?php echo (isset($options['paypal_email']) ? $options['paypal_email'] : ""); ?>">
							</td>
						</tr>
						<tr valign="top">
							<th scope="row">
				...
				<table class="form-table">
					<tbody>
						<tr valign="top">
							...
							<td>
								<input type="text" name="ew[notification-email]" value="<?php echo (isset($options['notification-email']) ? $options['notification-email'] : ""); ?>">
							</td>
						</tr>
						<tr valign="top">
							<th scope="row">
...
function ew_order_fields_callback( $obj, $box ) {
	...
	$fields = array("type", "delivery" => array("input" => "textarea"), "level", "changes_interval", "standard", "length", "premier", "fullname", "email", "phone", "mobile", "coupon", "question", "area_of_study", "referencing", "due", "suggestedsources", "requiredsources", "spss", "file", "address", "town", "county", "country", "postcode", "cost", "currency", "delivery_date", "status" => array("default" => "in-progress"), "is_paid" => array("default" => "0"), "attachments" => array("input" => "textarea"));
	foreach($fields as $field => $type) {
		...
		<p>
			...
			<input class="widefat" type="text" name="ew[<?php echo $field; ?>]" id="ew-<?php echo $field; ?>" value="<?php echo $val; ?>" />
			<?php } else if($input_type == "textarea") { ?>
			<textarea class="widefat" name="ew[<?php echo $field; ?>]" id="ew-<?php echo $field; ?>" rows="10"><?php echo $val; ?></textarea>
			<?php } ?>
		</p>
		<?php
	}
